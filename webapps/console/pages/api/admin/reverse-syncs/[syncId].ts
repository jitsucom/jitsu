...
export default createRoute()
  .GET({ auth: false, query: z.object({ syncId: z.string().min(1), workspaceId: z.string().min(1) }) })
  .handler(async ({ req, res, query }) => {
    ...
    try {
      const value = await db
        .prisma()
        .$transaction(tx => readReverseSync(tx, query.syncId, query.workspaceId), { isolationLevel: "RepeatableRead" });
