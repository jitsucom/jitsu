...
func NewRedisPool(config *RedisConfiguration) (*redis.Pool, error) {
	...
	if config.IsSentinelMode() {
		...
		dialFunc = getDialUrlFunc(config, options)
	} else {
		options = append(options, redis.DialPassword(config.password))
		dialFunc = getDialTcpFunc(config, options)
	}
...
func getDialTcpFunc(config *RedisConfiguration, options []redis.DialOption) func() (redis.Conn, error) {
...
func getDialUrlFunc(config *RedisConfiguration, options []redis.DialOption) func() (redis.Conn, error) {
	return func() (redis.Conn, error) {
