...
func TestEventEndpoint(t *testing.T) {
	...
	tests := []test.Integration{
		...
		{
			"IP three octets test",
			"/api/v1/event?token=c2stoken&privacy_policy=ip-three-octets",
			"test_data/event_input_2.0.json",
			"test_data/fact_output_ip_three_octets.json",
			"",
			http.StatusOK,
			"",
		},
		{
			"Cookie-less and ip three octets test",
			"/api/v1/event?token=c2stoken&cookie_less=true&privacy_policy=ip-three-octets",
			"test_data/event_input_2.0.json",
			"test_data/fact_output_cookie_less_ip_three_octets.json",
			"",
			http.StatusOK,
			"",
		},
