...
func TestSegmentAPIEvents(t *testing.T) {
	...
	sendSegmentRequests(t, "http://"+testSuite.HTTPAuthority()+"/api/v1/segment")

	time.Sleep(1 * time.Second)

	actualBytes := logging.InstanceMock.Data

	expectedAllBytes, err := ioutil.ReadFile("test_data/segment_api_events_output_2.0.json")
	require.NoError(t, err)
...
func TestSegmentCompatAPIEvents(t *testing.T) {
	uuid.InitMock()
	binding.EnableDecoderUseNumber = true
	...
	sendSegmentRequests(t, "http://"+testSuite.HTTPAuthority()+"/api/v1/segment/compat")
