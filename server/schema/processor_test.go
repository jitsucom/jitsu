...
func TestProcessFilePayload(t *testing.T) {
	...
	p, err := NewProcessor("test", "google_analytics", `{{if .event_type}}{{if eq .event_type "skipped"}}{{else}}{{.event_type}}_{{._timestamp.Format "2006_01"}}{{end}}{{else}}{{.event_type}}_{{._timestamp.Format "2006_01"}}{{end}}`, "", &DummyMapper{}, []enrichment.Rule{}, NewFlattener(nil), NewTypeResolver(), false, identifiers.NewUniqueID("/eventn_ctx/event_id"), 0)
	require.NoError(t, err)
...
func TestProcessFact(t *testing.T) {
	...
	p, err := NewProcessor("test", "google_analytics",`events_{{._timestamp.Format "2006_01"}}`, "", fieldMapper, []enrichment.Rule{uaRule, ipRule}, NewFlattener(nil), NewTypeResolver(), false, identifiers.NewUniqueID("/eventn_ctx/event_id"), 20)
...
`
	p, err := NewProcessor("test", "google_analytics",`events`, transormExpression, fieldMapper, []enrichment.Rule{}, NewFlattener(nil), NewTypeResolver(), false, identifiers.NewUniqueID("/eventn_ctx/event_id"), 20)
