	...
	for streamName, stream := range representation.Streams {
		...
		for _, storage := range rs.destinations {
			...
			if err != nil {
				...
				metrics.ErrorSourceEvents(rs.task.SourceType, rs.task.Source, storage.Type(), storage.ID(), rowsCount)
				metrics.ErrorObjects(rs.task.SourceType, rs.task.Source, rowsCount)
			...
			metrics.SuccessSourceEvents(rs.task.SourceType, rs.task.Source, storage.Type(), storage.ID(), rowsCount)
			metrics.SuccessObjects(rs.task.SourceType, rs.task.Source, rowsCount)
