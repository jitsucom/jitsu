...
func (te *TaskExecutor) execute(i interface{}) {
	...
	te.triggerStatusEvents(task, taskLogger)
}

func (te *TaskExecutor) triggerStatusEvents(task *meta.Task, taskLogger *TaskLogger) {
	...
	if err := te.MultiplexingService.AcceptRequest([]events.Event{success}, multiplexing.System); err != nil {
		logging.Error(err)
		taskLogger.ERROR(err.Error())
	} else {
		taskLogger.INFO("Successfully emitted task status events")
	}
}
