...
func (p *Postgres) Update(eventContext *adapters.EventContext) error {
	_, tableHelper := p.getAdapters()
	...
	start := timestamp.Now()
	if err = p.adapter.Update(dbSchema, processedObject, p.uniqueIDField.GetFlatFieldName(), p.uniqueIDField.Extract(processedObject)); err != nil {
		return err
	}
