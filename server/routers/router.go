...
func SetupRouter(adminToken string, metaStorage meta.Storage, destinations *destinations.Service, sourcesService *sources.Service, taskService *synchronization.TaskService,
	...
	eventsCache *caching.EventsCache, segmentEndpointFieldMapper events.Mapper) *gin.Engine {
	...
	apiV1 := router.Group("/api/v1")
	{
		...
		apiV1.POST("/segment/v1/batch", middleware.TokenFuncAuth(segmentHandler.PostHandler, appconfig.Instance.AuthorizationService.GetClientOrigins, ""))
		apiV1.POST("/segment", middleware.TokenFuncAuth(segmentHandler.PostHandler, appconfig.Instance.AuthorizationService.GetClientOrigins, ""))
