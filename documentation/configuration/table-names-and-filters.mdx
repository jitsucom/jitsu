...
**Jitsu** supports splitting events stream into different tables in DWH based on event JSON payload. It works by applying [GO text template](https://golang.org/pkg/text/template/) or [JavaScript expressions](docs/configuration/javascript-expressions)  to incoming JSON objects. It can select a table name or pass a constant or even filter certain events from the destination. It is configured in the destination section and each destination has its own rules.
...
destinations:
  destination_1:
    data_layout:
      ...
      table_name_template: '{{.event_type}}_{{._timestamp.Format "2006_01"}}' #Go template
      table_name_template: '`${_.event_type}_${_._timestamp.Format("2006_01")}`' #JavaScript
...
<Hint>
    If <code inline={true}>table_name_template</code> returns an empty string or null then the event will be skipped.
    Use <a href="/docs/other-features/admin-endpoints#apiv1templatesevaluate">Jitsu Template Evaluation endpoint</a> for testing GO text/template expression.
</Hint>
...
destinations:
  destination_1:
    data_layout:
      table_name_template: '{{if eq .event_type "pageview"}}{{else}}{{.event_type}}{{end}}' #Go template
