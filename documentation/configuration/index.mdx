...
If **Jitsu Server** is deployed in a standalone mode, it should be configured via single YAML file
...
**Jitsu Server** can be used either with [Redis](https://redis.io/) or not. Redis version 5 and higher is supported.
Storage is required for some features (but if youwould like to work without them, then Redis configuration can be omitted):
