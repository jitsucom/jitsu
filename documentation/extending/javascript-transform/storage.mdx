...
```typescript
type KeyValue = {
  // set value for a key. You can provide 'opts' object with 'ttlMs' - time-to-live value in milliseconds.
  // Value will be automatically deleted after provided value of milliseconds.
  // If 'ttlMs' is not provided, default TTL value configured on storage level will be used. 
