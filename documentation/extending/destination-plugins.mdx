...
```shell
yarn build && yarn validate-config -file config.json
```
...
```shell
yarn build && yarn exec --file event.json --config-object '{"webhookUrl":"https://hooks.slack.com/services/ABC/XYZ/etc", "eventTypes":"registration,error", "messageTemplate": "Important event of type: ${event_type}"}'
```
or with config from file:
```shell
yarn build && yarn exec --file event.json --config config.json
```
