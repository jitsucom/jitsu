...
To make concrete implementation of these types we need to define types for Source config and Stream config.
...
Source extension need a way to tell Jitsu what Stream types it supports and what parameters are required to configure these Streams
...
| **clear_stream**    | `clearStream`            | no              | Truncates table associated with stream in target destinations.<br/>`clear_stream` message must be the first message in transaction and can be used only once per stream.                                                                                                                                                                  |
| **state**           | `changeState`            | any object      | Stores provided object in persistent storage.                                                                                                                                                                                                                                                                                             |
| **new_transaction** | `newTransaction`         | no              | Starts a new transaction. Previous transaction will be committed to the destination. All consequent messages will be processed in the new transaction.<br/>First transaction is started implicitly.                                                                                                                                       |
...
Incremental mode usually requires Source to save some data between run, e.g. ID or _timestamp of last loaded data record.
To save data between run use `state` message.

#### Refresh window
Some third party services may update data during day and other service may even update data for longer periods like 30-day
...
To test that your source work correctly with state you can pass state object to the `execute` script using `-t` parameter:
```shell
yarn execute -c '{apiKey: "keyMy", baseId:"app1234"}' -s '{stream: "table", tableId:"tblMy"}'  -t '{run_number: 1}'
```

### Record $id
Usually Source data has some ID associated with its entities.
When this is no the case you can use helper function `buildSignatureId`  from `@jitsu/jlib/lib/sources-lib`
