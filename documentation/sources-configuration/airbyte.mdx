...
<Hint>
    Airbyte-based sources are available in <code inline={true}>beta</code> version only.{' '}
</Hint>
...
a stream of JSON objects as well. Depending on

<Hint>
    Make sure Jitsu has access to docker. If you've deployed Jitsu with Docker - you should mount <code inline={true}>/var/run/docker.sock:/var/run/docker.sock</code>.
...
of generic [source configuration](http://localhost:3000/docs/sources-configuration) - `type: airbyte` indicates that the source should be handled
by Airbyte executor
