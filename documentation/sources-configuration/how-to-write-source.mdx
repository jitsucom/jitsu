...
### Driver loading
For driver loading you should add implicit import with newly created driver implementation in `server/drivers/factory.go`
```go
import (
...
- implicit driver loading from [Driver loading](#driverloading) section to `server/drivers/factory.go` imports
