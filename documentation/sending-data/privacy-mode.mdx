...
To switch on privacy mode use `privacy_policy` parameter (or `data-privacy-policy`) following configuration parameter:
...
<Hint>
  ...
  analytics, so they fall under analytics (or functional) cookies category.
  <br /><br />
  The good idea is to switch <code inline="true">cookie_policy</code> and <code inline="true">ip_policy</code> to <code inline="true">strict</code> value only for those users who didn't consent to cookies or ip address.
</Hint>
