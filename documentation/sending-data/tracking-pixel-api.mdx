...
Jitsu has a [Tracking Pixel API](#apiv1pgif) endpoint for tracking email opens, impressions of advertisements and other cases when:
...
<APIParam name={"token"} dataType="string" required={false} type="queryString" description="Client or Server secret token. Also might be provided in data JSON object."/>
...
<h4>Request with data parameter</h4>
...
Request:
```bash
HTTP GET https://t.jitsu.com?data=ewogICJ0b2tlbiI6ICJjbGllbnRfc2VjcmV0IiwKICAidXNlciI6IHsKICAgICJhbm9ueW1vdXNfaWQiOiAiMTIzIgogIH0KfQ==
```

<h4> Request with data parameter and paths</h4>
...
Request:
```bash
HTTP GET https://t.jitsu.com?event_type=email_open&utm_campaign=cpa_10000&user.context.flag=true&data=ewogICJ0b2tlbiI6ICJjbGllbnRfc2VjcmV0IiwKICAidXNlciI6IHsKICAgICJhbm9ueW1vdXNfaWQiOiAiMTIzIgogIH0KfQ==
```
