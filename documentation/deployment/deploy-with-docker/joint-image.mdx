...
<APIParam small={true}  name="UI_AUTH_ACCESS_SECRET" dataType="string" required={true} type="Docker Env Var">
    ...
    Feel free to rotate them, change in the settings will force logout of of Configurator
</APIParam>
...
<APIParam small={true} name="USER_RECOGNITION_TTL_MINUTES" dataType="number" required={false} type="Docker Env Var">
  Default value is 7. This setting controls when user events are evicted from user recognition cache. Read
