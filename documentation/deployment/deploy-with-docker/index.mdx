...
`@jitsu/configurator` is an optional component. Jitsu Server can be configured with [YML file](http://localhost:3000/docs/configuration).

[Heroku deployment](http://localhost:3000/docs/deployment/deploy-on-heroku) users `@jitsucom/jitsu` under the hood. [Docker compose](/docs/deployment/deploy-with-docker#run-dockercompose)
