...
<Hint>
  Jitsu Server will also keep events logs in <code inline={true}>&lt;server_log_dir&gt;</code>.
</Hint>

<Hint>
  Since Jitsu uses Docker out of Docker approach for running Airbyte connectors, please add mounting docker socket: <code inline={true}>-v /var/run/docker.sock:/var/run/docker.sock</code>.
  If you don't use Airbyte sources, it isn't required.
</Hint>
