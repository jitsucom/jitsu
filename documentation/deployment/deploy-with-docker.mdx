...
* Create a `<server_log_dir>` and a `<configurator_log_dir>`. It will be used as Docker mount directory for keeping Jitsu logs.
...
Please, refer `<server_log_dir>` and `<configurator_log_dir>` by its absolute path. Use `$PWD` macro if necessary. Example:
...
<Hint>
  ...
  Jitsu Configurator address. Otherwise Jitsu Server doesn't take into account configuration from UI.
</Hint>
