...
[user recognition](/docs/other-features/retrospective-user-recognition)) and other monitor tasks.
...
### Coordination with etcd (legacy)

Jitsu Server can use `etcd` as a coordination service instead of Redis. It will still require redis to run [user recognition](/docs/other-features/retrospective-user-recognition)
and calculating statistics. We do not recommend set up new clusters with etcd (we do plan to support etcd as an alternative coordination services in the future)
...
requires adding 'sentinel_master_name' key to configuration
and explicitly specifying sentinel port
...
meta:
  storage:
    redis:
      host: your_redis_host # eg: localhost, redis-headless.redis.svc.cluster.local
      port: 26379
