...
const AmplitudeDestination: JitsuFunction<AnalyticsServerEvent, AmplitudeDestinationConfig> = async (
  ...
  } else if ((event.type === "page" || event.type === "track") && (event.userId || props.enableAnonymousUserProfiles)) {
    let geoObj: any = {};
    if (geo) {
      geoObj = {
  ...
  if (payload) {
    ...
    if (res.status === 200) {
      ...
      log.error(`Amplitude ${event.type} Error: ${res.status} message: ${await res.text()}`);
    }
  }
};
