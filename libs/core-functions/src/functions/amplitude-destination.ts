...
import { createFetchWrapper, createFunctionLogger, eventTimeSafeMs, JitsuFunctionWrapper } from "./lib";

const AmplitudeDestination: JitsuFunctionWrapper<AnalyticsServerEvent, AmplitudeDestinationConfig> = (
  chainCtx,
  funcCtx
) => {
  ...
  const func: JitsuFunction<AnalyticsServerEvent> = async (event, { geo, ua, ...ctx }) => {
    const props = funcCtx.props;
    try {
      const deviceId = event.anonymousId;
      let sessionId: number | undefined = undefined;
      if (deviceId) {
        const ttlStore = chainCtx.store;
