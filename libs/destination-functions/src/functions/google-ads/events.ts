...
import { GoogleAdsCredentials } from "./credentials";
...
const DATA_MANAGER_URL = `${dataManagerBaseUrl}events:ingest`;
...
const GoogleAdsDestination: JitsuFunction<AnalyticsServerEvent, GoogleAdsCredentials> = async (event, ctx) => {
  ...
  const url = useDataManager ? DATA_MANAGER_URL : `${googleAdsBaseUrl}customers/${customerId}:${legacyMethod}`;
  ...
  const result = await ctx.fetch(url, {
    method: "POST",
    headers: useDataManager
      ? dataManagerHeaders(accessToken)
