...
import { contactIdentifiers, values, userIdentifierSchema, hex, fail } from "../shared/identifiers";
...
import { dataManagerBaseUrl as baseUrl, dataManagerHeaders } from "../clients/data-manager";
  ...
  async function request(
    ...
    try {
      const response = await ctx.fetch(`${baseUrl}${path}`, {
        ...
        redirect: "error",
        signal: ctx.signal,
        headers: dataManagerHeaders(token, loginCustomerId),
