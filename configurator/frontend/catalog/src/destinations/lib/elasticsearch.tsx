...
const elasticsearchDestination: Destination = {
    ...
    parameters: [
        ...
        },
        {
            ...
            documentation: (
                <>
                    Send anonymous data to Elasticsearch if true or all data including user data if false.
                </>
            ),
        },
        {
        ...
        },
        {
            ...
            required: true,
            type: stringType,
            documentation: <>If the Elasticsearch security features are enabled, you must have the following index privileges for the target data stream, index, or index alias:<br />
                To add or overwrite a document using the PUT {`/<target>/_doc/<_id>`} request format, you must have the create, index, or write index privilege.<br />
                To add a document using the POST {`/<target>/_doc/`}, PUT {`/<target>/_create/<_id>`}, or POST {`/<target>/_create/<_id>`} request formats, you must have the create_doc, create, index, or write index privilege.<br />
