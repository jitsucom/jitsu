...
export const OnboardingTourReceiveEvent: React.FC<Props> = function({
  ...
  return (<div className={styles.mainContainer}>
    ...
    <div className={styles.controlsContainer}>
      <Button type="ghost" size="large" className={styles.withButtonsMargins} onClick={handleClickGoBack}>{'Back to Instructions'}</Button>
    </div>
    <div className={styles.skipButtonContainer}>
      <Button type="text" hidden={skipIsHidden} className={styles.withButtonsMargins} onClick={handleClickSkip}>{'I want to test it later '}</Button>
    </div>
  </div>);
}