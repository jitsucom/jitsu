...
const EventsView: React.FC<{ event: Event; className?: string; allDestinations: Record<string, DestinationData> }> = ({
  event,
  allDestinations,
  className,
}) => {
      ...
      {Object.entries(event.destinationResults).map(([destinationId, results]) => {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (result.status === "error") {
            ...
            display = (
              <div className="font-monospace flex justify-center items-center">
                Event was skipped: {JSON.stringify(result.rawJson)}
              </div>
            )
          } else {
          ...
          panes.push(
            <Tabs.TabPane
              tab={
                <TabTitle error={error} icon={destinationType.ui.icon}>
...
const EventsList: React.FC<{ destinationsFilter: string[]; reloadCount: number }> = ({
  destinationsFilter,
  reloadCount,
}) => {
  ...
  return (
    <div className="w-full">
      {events.map(event => {
        ...
        const hasFailedEvent = !!Object.values(event.destinationResults)
          .flatMap(res => res)
          .find(dest => dest.status === "error")
        const hasPendingEvent = !!Object.values(event.destinationResults)
          .flatMap(res => res)
          .find(dest => dest.status === "pending")
        return (
...
const DestinationsFilter: React.FC<{
  onChange: (destinations: string[]) => void
  allDestinations: DestinationData[]
  initialFilter?: string[]
}> = ({ initialFilter, onChange, allDestinations }) => {
        ...
        <div className="w-96 h-96 overflow-y-hidden overflow-hidden pr-6">
          <div className="flex pb-4">
            <div className="flex-grow">
              <Button type="link" size="small" onClick={() => setSelectedIds(allDestinations.map(dst => dst._uid))}>
                Select All
              </Button>
          ...
          <div className="flex flex-col h-80 overflow-y-auto pr-2">
            {allDestinations.map(dst => {
              ...
              return (
                <div className="flex space-y-2" key={dst._uid}>
                  <div onClick={toggleCheckBox} className="flex flex-nowrap items-center space-x-2 w-96">
                    ...
                    <span>
                      <MaxLen len={40}>{dst.displayName || dst._id}</MaxLen>
                    </span>
                  </div>
                  <div>
                    <Checkbox checked={selectedIds.includes(dst._uid)} onClick={toggleCheckBox} />
                  </div>
                </div>
              )
            })}
