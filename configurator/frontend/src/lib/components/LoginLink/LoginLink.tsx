...
export function LoginLink() {
  ...
  if (error) {
    return <div className="w-2/4 mx-auto mt-16">
      <Card title={<>
        <h2 className="text-error"><CloseCircleOutlined /> Error occurred</h2>
      </>} bordered={false}>
    ...
    return <div className="w-2/4 mx-auto mt-16">
      <Card title={<>
        <h2 className="text-success"><CheckCircleOutlined/> Check your email!</h2>
      </>} bordered={false}>
        <div className="text-xl">
          Magic login link has been sent to <b>{email}</b>. Check your inbox to login
          to Jitsu.Cloud
        </div>
      </Card>
    </div>
    ...
    userService.loginWithLink(email, window.location.href)
      .then((result) => {
    ...
    userService.sendLoginLink(email as string).then(() => {
      setEmailSent(true);
      setLoading(null);
    }).catch(error => {
      setError(error);
