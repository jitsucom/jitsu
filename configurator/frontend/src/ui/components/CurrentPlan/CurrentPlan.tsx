...
export const CurrentPlan: React.FC<CurrentPlanProps> = ({
  ...
  const usagaPct =
    (planStatus.usage.events / planStatus.currentPlan.quota.events) *
    100;
  return (
    <>
      <div>
            ...
            <tbody>
              <tr>
                ...
                <td className={styles.limitValue}>
                  {numberFormat(planStatus.usage.events)} /{' '}
                  {numberFormat(planStatus.currentPlan.quota.events)}
                </td>
              </tr>
              <tr>
                ...
                <td className={styles.limitValue}>
                  ...
                  {numberFormat(planStatus.currentPlan.quota.sources)}
                </td>
              </tr>
              <tr>
        ...
        {planStatus.currentPlan.id !== 'free' && <div className="text-xs">The plan {planStatus.autorenew ? 'renews' : 'expires'} on{' '}
          <b>{planStatus.expiration.format('MMM Do, YYYY')}</b></div>}
...
export const UpgradePlan: React.FC<{
  ...
  return <>
    ...
    <div className={styles.plan}>
      <div>
        <Button size="large" className={styles.planButton} type="primary" onClick={() => handleProceedToCheckout('growth')}>Upgrade to GROWTH</Button>
        <div className={styles.planPrice}>$99 / month</div>
      </div>
      <div>
        <Button size="large" className={styles.planButton} type="primary" onClick={() => handleProceedToCheckout('premium')}>Upgrade to PREMIUM</Button>
        <div className={styles.planPrice}>$299 / month</div>
      </div>
      <div>
        <Button size="large" type="primary" className={styles.planButton}>Upgrade to Enterprise</Button>
        <div className={styles.planPrice}>
          Custom. Email sales@jitsu.com
        </div>
      </div>
    </div>
    <div className="flex justify-center pt-3 space-x-3">
      <a target="_blank" href="https://jitsu.com/pricing" rel="noreferrer">Explore pricing options</a>{planStatus.stripeCustomerId &&
    <><span>•</span><a
      href={generateCustomerPortalLink({
        project_id: services.activeProject.id,
        user_email: services.userService.getUser().email,
    ...
    <div className="flex justify-center pt-3 text-secondaryText text-sm">
      Ask sales@jitsu.com for discounts :)
    </div>
