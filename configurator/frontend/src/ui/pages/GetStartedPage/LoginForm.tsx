...
export function LoginForm({ supportOauth }) {
  ...
  const googleSignIn = async() => {
    try {
      ...
      message.destroy();
      services.analyticsService.track('failed_app_login', { user: { email: null }, login_type: 'google', ...getErrorPayload(error) });
  ...
  const passwordSignIn = async(values) => {
    ...
    try {
      ...
      message.destroy();
      await services.analyticsService.track('failed_app_login', { user: { email }, login_type: 'password', ...getErrorPayload(error) });
  ...
  return <div>
    ...
    <div className="block lg:hidden mt-6 text-center mb-6 font-bold">New to Jitsu? <a onClick={() => history.push('/signup')}>Sign up</a></div>
    <Form
      name="signup-form"
    ...
    >
      ...
      <div className="text-center">
        ...
        <div><a onClick={() => setShowPasswordResetForm(true)}>Forgot password?</a></div>
        {supportOauth && <><div className="mb-4 mt-4">Or sign in with</div>
          <div className="flex space-x-4 justify-center">
            <Button
              size="large"
              onClick={githubSignIn}
              icon={<Icon component={() => <img className="h-4 align-baseline" src={githubLogo}/>}/>}>Github</Button>
            <Button
              size="large"
              onClick={googleSignIn}
              icon={<Icon component={() => <img className="h-4 align-baseline" src={googleLogo}/>}/>}>Google</Button>
          </div></>}
      </div>
    </Form></div>
}
