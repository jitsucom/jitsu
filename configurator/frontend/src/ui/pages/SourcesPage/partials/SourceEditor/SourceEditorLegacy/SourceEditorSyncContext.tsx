...
function assertContextIsValid(context: Partial<ContextParams>): asserts context is ContextParams {
  if (typeof context.isLoadingConfigParameters === "undefined") throwErrorOutOfContext()
}

const throwErrorOutOfContext: ThrowError = () => {
  throw new Error("attempted to use SourceEditorSyncContext outside of the context")
}
...
export const WithSourceEditorSyncContext: React.FC<ContextProps> = ({ children, connectorSource }) => {
  const [isLoadingConfigParameters, setIsLoadingConfigParameters] = useState<boolean>(
    connectorSource.hasLoadableConfigParameters
  )
  return (
