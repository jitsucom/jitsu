...
const SourceEditorCollections = ({
  ...
  const [chosenTypes, setChosenTypes] = useState<{ [key: number]: string }>(
    initialValues.collections
      ? initialValues.collections?.reduce(
          (accumulator: any, value: CollectionSource, index: number) => {
  ...
  const generateReportName = useCallback(
    (index: number) => {
      ...
      const blankName = `${connectorSource.id}_${
        collections
    ...
    (index: number) =>
      connectorSource.collectionParameters?.filter(
        ({ applyOnlyTo }: CollectionParameter) =>
          !applyOnlyTo || applyOnlyTo === chosenTypes[index]
      ),
                      ...
                      <Row>
                        <Col span={16}>
                            ...
                            rules={
                              connectorSource.collectionTypes.length > 1
                                ? [
                                    {
                                      required: true,
                                      message: 'You have to choose report type'
                                    }
                                  ]
                                : undefined
                            }
                          >
                            ...
                            >
                              {connectorSource.collectionTypes.map(
                                (type: string) => (
                          ...
                          <Form.Item
                            ...
                            label={
                              <LabelWithTooltip
                                documentation={
                                  <>
                                    Name of the report. Will be used as table
                                    name prefixed with source_id. Table name
                                    will be:{' '}
                                    <CodeInline>
                              ...
                              },
                              {
                                validator: (rule: any, value: string) => {
                                  ...
                                  const isError = formValues.collections
                                    .map(
                                      (collection, index) =>
                                        index !== field.name && collection.name
                                    )
                                    .includes(value);
