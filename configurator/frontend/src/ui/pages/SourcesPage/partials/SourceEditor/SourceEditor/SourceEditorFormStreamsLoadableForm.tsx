...
const getAirbyteStreamsUniqueIds = (streams: AirbyteStreamData[]): string[] =>
  streams.map(getAirbyteStreamUniqueId);
...
const SourceEditorFormStreamsLoadableForm = ({
  ...
  const initiallyCheckedStreams = selectAllFieldsByDefault
    ? allStreams
    : initiallySelectedStreams ?? [];
  ...
  const [checked, setChecked] = useState<Set<string>>(
    new Set(getAirbyteStreamsUniqueIds(initiallySelectedStreams))
  );

  const handleAddStream = (stream: AirbyteStreamData) => {
    addStream(STREAMS_CATALOG_PATH, stream);
    setChecked((checkedStreams) =>
      new Set(checkedStreams).add(getAirbyteStreamUniqueId(stream))
    );
  };

  const handleRemoveStream = (stream: AirbyteStreamData) => {
    removeStream(STREAMS_CATALOG_PATH, stream);
    setChecked((checkedStreams) => {
  ...
  const handleSetStreams = (streams: AirbyteStreamData[]) => {
    setStreams(STREAMS_CATALOG_PATH, streams);
    setChecked(new Set(getAirbyteStreamsUniqueIds(streams)));
  };

  const handleToggleStream = useCallback(
    (checked: boolean, stream: AirbyteStreamData): void => {
  ...
  const handleChangeStreamSyncMode = (
    mode: string,
    ...
    updateStream(STREAMS_CATALOG_PATH, newStream);
  };
  ...
  useEffect(() => {
    handleSetStreams(initiallyCheckedStreams);
  }, []);

  return (
    <div
      className={`h-full w-full flex-col items-stretch ${
        hide ? 'hidden' : 'flex'
      }`}
    >
      <div className="flex items-center mb-4">
        ...
        />
        <div className="flex flex-grow-0 flex-shrink-0 items-center ml-4">
              ...
              return (
                <Collapse.Panel
                      ...
                      >
                        ...
                        {showSyncModeSelection ? (
                          <Select
                            size="small"
                            disabled={
                              !checked.has(getAirbyteStreamUniqueId(streamData))
                            }
                            defaultValue={
                  ...
                  extra={
                    <Switch
                      checked={checked.has(
                        getAirbyteStreamUniqueId(streamData)
                      )}
                      defaultChecked={
                        selectAllFieldsByDefault
                          ? true
...
SourceEditorFormStreamsLoadableForm.displayName =
